...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn("p-3 bg-[#337B5B] border border-black border-collapse", className)}
      classNames={{
        ...
        nav_button_previous: "absolute left-0 text-white border-black border-collapse hover:bg-[#255941] hover:border-[#255941]",
        nav_button_next: "absolute right-0 text-white border-black border-collapse hover:bg-[#255941] hover:border-[#255941]",
        table: "w-full border-collapse border border-black border-collapse capitalize",
        head_row: "flex",
        head_cell: "rounded-md w-9 font-normal text-[0.8rem]",
        row: "flex w-full mt-2",
        cell: "h-9 w-9 text-center text-sm p-0 relative hover:border-black border-collapse hover:border [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
        day: cn(
